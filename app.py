...
fig_bar.update_layout(
    barmode="stack",
    title=None,  # Titel in Plotly entfernt, um Doppelung zu verhindern
...
# ==========================================
# 10. HISTORICAL TIMELINE ENGINE (EXAKTER SEC MERGE)
# ==========================================
st.markdown("---")
...
# 2. Physische BTC-Deckung pro Aktie in Sats (Exakt nach SEC 8-K / 10-Q)
merged_df['internal_btc_sats'] = (user_shares * (merged_df['btc_holdings'] / merged_df['shares_out'])) * SATS_PER_BTC

# 3. Historische Cash-Reserve in Sats (Exakte USD Balance aus SEC-Filings / BTC-Kurs des Tages)
...
# Total Asset Base = Physische BTC + Physisches Cash (in Sats)
...
# 4. Total Substance (BTC + Historische Cash Reserve - Grün Gestrichelt)
fig_line.add_trace(go.Scatter(
    x=merged_df.index,
...
with col_f2:
    with st.expander(t["sources_title"]):
        st.markdown(t["footer_sources_md"])    ("2021-12-09", 8436, 112000000, 90000000, "SEC Form 8-K (09.12.2021)"),
    ("2021-12-30", 1913, 112000000, 90000000, "SEC Form 8-K (30.12.2021)"),
    ("2022-01-31", 660, 112000000, 80000000, "SEC Form 8-K (31.01.2022)"),
    ("2022-04-05", 4167, 113000000, 70000000, "SEC Form 8-K (05.04.2022)"),
    ("2022-06-29", 481, 113000000, 60000000, "SEC Form 8-K (29.06.2022)"),
